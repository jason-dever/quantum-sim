...
Riemann sum $\ket{\hat{\psi}(t)} = \sum_{k = -\frac{d}{\Delta x}}^{\frac{d-1}{\Delta x}} a_k \ket{k \Delta x}$
($a_k \in \mathbb{C}$). We let $N = \frac{2d}{\Delta x}$ denote the number of grid positions.
...
This gives the solution in Fourier space $\mathcal{F}(\psi)(\xi, t) = e^{-i t \xi^2}\mathcal{F}(\psi)(\xi, 0)$.
Applying the inverse Fourier transform, we have $\psi(x, t) = \mathcal{F}^{-1}\left(e^{-i t \xi^2} \mathcal{F}(\psi)(\xi, 0)\right)$.
...
choice of convention, $QFT^\dag: \ket{k} \mapsto \frac{1}{\sqrt{N}} \sum_{j=0}^{N-1} e^\frac{-2\pi ijk}{N} \ket{j}$
...
\begin{align*}
    (2k-N)^2 &= I^2 + 2 \sum_{j=0}^{n-1} 2^j Z_j + \left( \sum_{j=0}^{n-1} 2^j Z_j \right) \left( \sum_{l=0}^{n-1} 2^l Z_l \right) \\
             ...
             &\overset{(*)}{=} I + \sum_{j=0}^{n-1} 2^{j+1} Z_j + \sum_{j=0}^{n-1} \sum_{l=j}^{n-1} 2^{j+l+1} Z_j Z_l \text{\quad since $Z_j Z_l = Z_l Z_j$}
\end{align*}
...
\begin{align*}
    e^{-i \xi_k^2 \Delta t} &= e^{-i \left(\frac{\pi}{L}(2k-N)\right)^2 \Delta t} \\
                            ...
                            &= \prod_{j=0}^{n-1} R_{Z_j}\left(2^{j+2} \frac{\pi^2}{L^2} \Delta t\right) \prod_{j=0}^{n-1} \prod_{l=j+1}^{n-1} R_{jl}\left(2^{j+l+2} \frac{\pi^2}{L^2} \Delta t\right) 
                            \text{\quad by definition}
\end{align*}
$(*)$ up to global phase \newline
$R_z$ and $R_{zz}$ gates are easily accessible on a quantum computer. This is the
circuit returned by kinetic() of schrodinger.py.
