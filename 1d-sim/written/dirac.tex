...
\begin{align*}
    \exp{(-1)^s \frac{\pi}{L}(2k-N)\Delta t} &= \exp\left((-1)^{s+1} i N \frac{\pi}{L}\Delta t\right)\exp\left((-1)^s 2i \frac{\pi}{L} \Delta t k\right) \\
    &= \exp\left((-1)^{s+1} i N \frac{\pi}{L}\Delta t\right)\bigotimes_{j=0}^{n-1} P\left((-1)^s2^{j+1}\frac{\pi}{L} \Delta t \right)
\end{align*}
This gives us a relative phase difference of $e^{\frac{2\pi i N \Delta t}{L}}$ between
$\varphi_1$ and $\varphi_2$, which we compensate for by applying $P\left(\frac{-2\pi N \Delta t}{L}\right)$
to the spinor qubit. The quantum circuit we implement to apply the shift phase is below.
$\theta = \frac{\pi}{L} \Delta t$.
\begin{center}
\makebox[\textwidth][c]{%
\begin{quantikz}[wire types={q,q,n,q,q}, row sep={0.9cm,between origins}]
    \lstick{$\ket{k_0}$} & \gate{P(-\theta)} &&&&&& \gate{P(\theta)}&&&  \\
    \lstick{$\ket{k_1}$} && \gate{P(-2\theta)} &&&& \gate{P(2\theta)} &&&&       \\
    \lstick{$\vdots$}    &&&&&&&&&   &\\ 
    \lstick{$\ket{k_{N-1}}$} &&& \gate{P(-2^{N-1}\theta)} &  & \gate{P(2^{N-1}\theta)} &&&& & \\
    \lstick{$\ket{s}$}   & \ctrl{-4}  & \ctrl{-3}  & \ctrl{-1}  & \gate{X} & \ctrl{-1}  & \ctrl{-3}  & \ctrl{-4}  &  \gate{X} & \gate{e^{-2 N \theta}} &
\end{quantikz}%
}
\end{center}
...
and we have the state $\ket{\varphi} = \ket{s}\ket{\text{pos}} \propto \ket{0}\ket{\varphi_1} + \ket{1}{\ket{\varphi_2}}$
\begin{align*}
    \ket{\psi} &\propto \ket{0}\ket{\psi_1} + \ket{1} \ket{\psi_2} \\
               ...
               &= H\ket{s}\ket{\text{pos}}
\end{align*}
By a symmetric argument, we can transform between $\ket{\varphi}$ and $\ket{\psi}$
with a Hadamard gate on the spinor qubit. Thus our solution to $\partial_t \psi = -\sigma_x \partial_x \psi$
is given by simply applying $H$ to $\ket{s}$, following the work above for the linear transport,
then applying another $H$ to $\ket{s}$. This is the circuit returned by transport().
\end{document}
